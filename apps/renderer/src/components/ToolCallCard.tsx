...
export const ToolCallCard = ({ tool }: { tool: ToolCallView }) => {
  ...
  return (
    <div className="bg-surface-soft border border-surface-border border-l-2 border-l-brand rounded-md p-3 mt-2 font-mono text-xs">
      <div className="font-sans text-[11px] font-semibold text-brand uppercase tracking-wide mb-1.5 flex items-center gap-1.5">
        <Wrench size={12} aria-hidden={true} />
        {tool.name}
        {tool.status === "pending" && (
          <span className="ml-2 text-ink-soft normal-case font-normal">running…</span>
        )}
      </div>
