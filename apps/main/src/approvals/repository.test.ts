...
describe("ApprovalsRepository — CEO routing", () => {
  ...
  it("listPendingForStaleSweep returns only still-pending approvals with an agent", () => {
    const repo = createApprovalsRepository(d);
    const pending = repo.create({
      agentId: "a1",
      kind: "tool_call",
      payload: { tool_name: "Write", tool_input: {}, tool_use_id: "t-pending" },
    });
    const decided = repo.create({
      agentId: "a1",
      kind: "tool_call",
      payload: { tool_name: "Read", tool_input: {}, tool_use_id: "t-decided" },
    });
    repo.decide(decided.id, "approved", "user");

    const sweep = repo.listPendingForStaleSweep();
    expect(sweep.map((s) => s.id)).toEqual([pending.id]);
    expect(sweep[0]?.agentId).toBe("a1");
    expect(typeof sweep[0]?.createdAt).toBe("number");
  });
});
