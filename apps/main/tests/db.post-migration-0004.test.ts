...
const seedAgent = (
  ...
  ).run(id, companyId, opts.templateId ?? null, opts.skillsJson ?? "[]", opts.reportsTo ?? null);
};
...
describe("postMigration 0004 — seed roles + backfill", () => {
  ...
  it("seeded CEO role has Opus model and delegation+issues+inbox+chat+fs-read skills", () => {
    ...
    const ceo = db
      .prepare(
        "SELECT default_model, default_skills_json FROM role_templates WHERE id = 'role-ceo'",
      )
      .get() as { default_model: string; default_skills_json: string };
    expect(ceo.default_model).toBe("claude-opus-4-7");
    expect((JSON.parse(ceo.default_skills_json) as string[]).sort()).toEqual([
      "chat",
      "delegation",
      "fs-read",
      "inbox",
  ...
  it("backfills root CEO agent (reports_to IS NULL, template_id IS NULL) with role-ceo data", () => {
    ...
    expect((JSON.parse(row.skills_json) as string[]).sort()).toEqual([
      "chat",
      "delegation",
      "fs-read",
      "inbox",
  ...
  it("does not overwrite an agent that already has a template_id", () => {
    ...
    runPostMigration0004(db);
    const row = db.prepare("SELECT template_id, skills_json FROM agents WHERE id = 'a1'").get() as {
      template_id: string;
      skills_json: string;
    };
    expect(row.template_id).toBe("role-engineer");
    expect(JSON.parse(row.skills_json) as string[]).toEqual(["fs-read"]);
  });
