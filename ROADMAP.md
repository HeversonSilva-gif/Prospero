...
- **M13 ✅ COMPLETO** — Espinha de Outcome & Verificação. **6 PRs (A–F) mergeados 2026-05-19/05-20**: cada objetivo tem uma definição estruturada do que significa estar "pronto"; quando todas as tarefas terminam, os critérios são checados automaticamente — o objetivo só vira "concluído" se tudo passar, senão volta para "em andamento" com um aviso; os agentes podem checar/julgar critérios eles mesmos; cada empresa tem um **TELOS** — o norte do negócio — que passa a guiar todo agente; todo agente carrega no contexto **The Algorithm** — o loop operacional de 7 fases (observar → pensar → planejar → construir → executar → verificar → aprender) entregue como skill embutida; um **mapa estrutural de zonas de privacidade** declara as fronteiras entre empresas e entre agentes — cross-zone vira deny + auditoria + card no inbox (defense-in-depth sobre o sandbox CWD); o **PR-F de consolidação** fechou os follow-ups acumulados: validação da síntese do TELOS, ISA + TELOS no `company:export` (backup completo), broadcast UI no `criterion_judge`, polish da UI da Vitrine de Verificação, e `SECURITY.md` + `roadmap.html` atualizados.
- **1564 testes passing + 2 todo** · 0 lint/typecheck errors
- HEAD `main`: M13 PR-F (consolidação — último PR do milestone) mergeado (2026-05-20)

### ▸ Próximo

| Candidato | Escopo | Por quê |
|---|---|---|
| 🥇 **M14 Vitrine Matinal & Escada de Confiança** | Resumo diário de triagem + autonomia que compounda com track record verificado do M13. 2 peças, 4 PRs. Pré-req do M15 (Routines). | Fecha o "lado humano do loop" da V2 — depois de M11 (memória) + M12 (definição) + M13 (verificação), a próxima alavanca é a confiança humana crescendo no tempo. Spec em `docs/superpowers/specs/2026-05-18-m14-morning-showcase-trust-ladder-design.md`. |

**Recomendação:** começar M14 — toda a maquinaria do motor (M11+M12+M13) está pronta; a próxima janela de valor é o usuário se sentir no controle sem ficar revisando tudo.
...
| Concluído recentemente | **M13 PR-F — Consolidação** mergeado 2026-05-20 — **fecha o M13 inteiro (6/6 PRs)**. 8 itens em 10 commits diretos em `main`: (1) `validateTelos` rodando na síntese do TELOS — síntese malformada agora mostra erros inline (sem bloquear save); (2) `company:export` + `:import` carregam `isa.md` + `telos.md` como `artifacts` top-level (backwards-compat preservada; round-trip restaura paths no banco); (3) broadcast de UI quando `criterion_judge` (MCP) ou IPC do user fecha um goal — render atualiza ao vivo; (4) inbox card pra `security.zone_blocked` (migration 0031 — `inbox_items.kind` tinha CHECK constraint não previsto, plan flagou; 5-min de-dup, broadcast `INBOX_UPDATE`, sem CTA); (5) polish da Vitrine de Verificação (`IsaPanel.tsx` +139 LOC sob cap 200 — bucketing por status com failed-at-top, `STATUS_STYLES` map exhaustivo por `CriterionStatus`, `role="alert"` no error banner, novo `EmptyState` com hint); (6) `SECURITY.md` ganhou 2 seções (Containment zones + Verification as attack surface, com V2 hardening gap documentado); (7) `roadmap.html` (público) reflete M11+M12+M13 em tom leigo; (8) auditoria de não-regressão (1564 + 2 todo, zero footprint no system prompt do agente). 3 commits de follow-up dos reviews per-task (`4e30b0d` empty-body asymmetry + preload type drift; `09b1a6a` propagate gate errors no user judge path; `7cb1595` `broadcastInboxUpdate` no zone card). Antes: **M13 PR-A..PR-E** mergeados 2026-05-19/05-20. |
| Testes | **1564 passing + 2 todo**, 0 lint/typecheck errors |
| Commits no main | ~845 |
...
| Restante pra v1 | **Nada — v1 fechado em 2026-05-15.** M11 + M12 + **M13 fechados** (PR-A..F ✅). Próximo milestone V2: **M14 (Vitrine Matinal)**. |
