...
- **M14 🔄 EM ANDAMENTO** — Vitrine Matinal & Escada de Confiança. **PR-A (Escada — backend) + PR-B (Escada — UI) mergeados 2026-05-20**: cada funcionário tem um **degrau de confiança** (`novato → confiável → autônomo`) que sobe automaticamente conforme entrega objetivos verificados sem falhas. Um funcionário "confiável" deixa de pedir aprovação pra ler arquivos (Write/Edit/Bash continuam supervisionados); a subida pra "autônomo" (que liga o modo `auto`) **passa por você** via um card de inbox; rebaixamento é imediato em qualquer falha (segurança antes de UX). A UI agora **mostra o degrau atual** ao lado de cada agente (lista e header), tem um **painel de histórico de confiança** na aba Stats com todas as promoções/rebaixamentos/sugestões, e o card de inbox `trust_promotion_suggested` tem um botão Aprovar que aplica a promoção e liga o modo auto. Faltam PR-C (Vitrine Matinal) + PR-D (consolidação).
- **1625 testes passing + 2 todo** · 0 lint/typecheck errors
- HEAD `main`: M14 PR-B (Escada de Confiança — UI) mergeado (2026-05-20)

### ▸ Próximo

| Candidato | Escopo | Por quê |
|---|---|---|
| 🥇 **M14 Vitrine Matinal & Escada de Confiança** 🔄 EM ANDAMENTO | ✅ PR-A Escada backend (auto-aprovação read-only + motor reativo + IPC) · ✅ PR-B Escada UI (badge + painel histórico + card de aprovação) · PR-C Vitrine Matinal (briefing diário + manchete cacheada + virar rota inicial) · PR-D consolidação | Fecha o "lado humano do loop" da V2 — depois de M11 (memória) + M12 (definição) + M13 (verificação), a próxima alavanca é a confiança humana crescendo no tempo. Spec em `docs/superpowers/specs/2026-05-18-m14-vitrine-confianca-design.md`. |

**Recomendação:** seguir o M14 — PR-C (Vitrine Matinal) é o próximo PR. Vira a rota inicial do app pra um briefing diário ("Precisa de você" no topo + resumo do que rodou).
...
| Concluído recentemente | **M14 PR-B — Escada de Confiança (UI)** mergeado 2026-05-20. 7 commits inline em `main` (8 tasks no plano, audit sem commit): novo `TrustTierBadge` (chip com dot colorido + label i18n — `novato` ink-soft · `confiavel` semantic-success · `autonomo` brand · sem emojis, regra do projeto); slot do badge na lista `/agents` (sob `agent.role`) + no `AgentHeader` (ao lado do role chip); novo `TrustHistoryPanel` (consome `trust:get-history`, render reverse-chrono com `KIND_DOT` por evento, estados loading/empty/error explícitos) slotado na aba Stats; novo `TrustPromotionCard` que renderiza no Inbox quando `item.kind === "trust_promotion_suggested"` com botão "Aprovar promoção" chamando `trust:approve-promotion` (pending + error states locais); +14 chaves i18n em ambos os locales (`trust.tier.*`, `trust.badge.title.*`, `trust.history.*`, `trust.promotionCard.*`). Renderer-only — zero touch no main/orchestrator (grep confirmou). 0 testes novos (convenção do projeto não tem react-testing-library; parity test cobre i18n; visual fica pro smoke manual). Antes: **M14 PR-A** mergeado 2026-05-20 (Escada backend completa). |
...
| Restante pra v1 | **Nada — v1 fechado em 2026-05-15.** M11 + M12 + M13 fechados. **M14 em andamento** (PR-A ✅ · PR-B ✅ · PR-C/D restantes). |
