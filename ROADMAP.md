...
- **M13 🔄 EM ANDAMENTO** — Espinha de Outcome & Verificação. **PR-A (ISA) + PR-B (motor de verificação, B1+B2) + PR-C (TELOS) + PR-D (The Algorithm) + PR-E (Containment Zones) mergeados 2026-05-19/05-20**: cada objetivo tem uma definição estruturada do que significa estar "pronto"; quando todas as tarefas terminam, os critérios são checados automaticamente — o objetivo só vira "concluído" se tudo passar, senão volta para "em andamento" com um aviso; os agentes podem checar/julgar critérios eles mesmos; cada empresa tem um **TELOS** — o norte do negócio — que passa a guiar todo agente; todo agente carrega no contexto **The Algorithm** — o loop operacional de 7 fases (observar → pensar → planejar → construir → executar → verificar → aprender) entregue como skill embutida, com VERIFY ligado ao motor de verificação e LEARN alimentando o pipeline de aprendizado da empresa com sinal mais rico (quantas tentativas cada critério levou; falhas de verificação viram gatilho de derivação); e agora um **mapa estrutural de zonas de privacidade** declara as fronteiras entre empresas e entre agentes — um agente que tenta tocar um arquivo de outra empresa ou de outro agente leva deny no gate de segurança e fica registrado num evento de auditoria, com painel read-only em Settings. Falta PR-F consolidação.
- **1550 testes passing + 2 todo** · 0 lint/typecheck errors
- HEAD `main`: M13 PR-E (Containment Zones — privacidade estrutural cross-empresa) mergeado (2026-05-20)

### ▸ Próximo

| Candidato | Escopo | Por quê |
|---|---|---|
| 🥇 **M13 Espinha de Outcome & Verificação** 🔄 EM ANDAMENTO | ✅ PR-A ISA (definição estruturada de "pronto") · ✅ PR-B motor de verificação (checagem automática + gate de conclusão + ferramentas de verificação para os agentes) · ✅ PR-C TELOS (norte da empresa) · ✅ PR-D The Algorithm (loop operacional de 7 fases) · ✅ PR-E Containment Zones (privacidade estrutural cross-empresa) · PR-F consolidação | Concretiza a aposta "Enforced Outcomes" da V2 — torna o "feito" um fato verificável, não uma opinião. Doc de design em `docs/superpowers/specs/2026-05-18-m13-outcome-verification-spine-design.md`. |

**Recomendação:** seguir o M13 — PR-F (consolidação) é o último PR do milestone.
...
| Concluído recentemente | **M13 PR-E — Containment Zones** mergeado 2026-05-20. Defense-in-depth sobre o sandbox CWD do M6.1 — um classificador puro `zoneOf(absPath, userDataDir)` mapeia cada caminho sob `<userData>` numa zona (`company`/`agent`/`shared`/`system`) e uma regra `canAccess(actor, zone)` decide se o agente daquela empresa pode tocar aquele arquivo. O check entra no `evaluatePermission` logo depois do path-fence existente: caminhos fora do mapa de zonas seguem como antes (path-fence decide); caminhos dentro de zonas cross-company/cross-agent levam deny + auditoria via `security.zone_blocked`. Read-only — sem nova migration, sem nova MCP tool, zero impacto no prompt do agente. Settings ganha painel "Security zones" listando as zonas vivas + caminhos exemplo. IPC `security:list-zones` derivado do banco (uma row por empresa + uma por agente não terminado). 6 commits inline em `main`: action + zone module + thread userDataDir + gate hook + IPC handler + preload bridge + panel + slot Settings. **+29 testes** (zone classifier 20 + gate-zones 6 + handlers 3). Antes: **M13 PR-D — The Algorithm**, **PR-C — TELOS**, **PR-B2/B1 — motor de verificação**, **PR-A — ISA**, todos 2026-05-19. |
| Testes | **1550 passing + 2 todo**, 0 lint/typecheck errors |
| Commits no main | ~845 |
...
| Restante pra v1 | **Nada — v1 fechado em 2026-05-15.** M11 + M12 fechados. **M13 em andamento** (PR-A ✅ · PR-B ✅ · PR-C ✅ · PR-D ✅ · PR-E ✅, falta PR-F). |
