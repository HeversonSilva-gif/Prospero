...
- **M14 🔄 EM ANDAMENTO** — Vitrine Matinal & Escada de Confiança. **PR-A + PR-B (Escada — backend + UI) + PR-C (Vitrine Matinal) mergeados 2026-05-20**: cada funcionário tem um **degrau de confiança** (`novato → confiável → autônomo`) que sobe automaticamente conforme entrega objetivos verificados (badge na lista + header + painel de histórico na Stats tab + card de aprovação no Inbox). E agora a **Vitrine Matinal vira a tela inicial do app**: um briefing diário com "Precisa de você" no topo (aprovações pendentes, verificações falhadas, promoções sugeridas, agentes travados), uma faixa colapsável com o resto (entregues / falharam / em andamento / aprendidos), e um rodapé com o custo da noite. A manchete é gerada por IA (`claude -p` curto, cacheada por hash dos contadores) com fallback determinístico — abrir a Vitrine 5× no mesmo estado custa zero. Falta PR-D (consolidação).
- **1641 testes passing + 2 todo** · 0 lint/typecheck errors
- HEAD `main`: M14 PR-C (Vitrine Matinal) mergeado (2026-05-20)

### ▸ Próximo

| Candidato | Escopo | Por quê |
|---|---|---|
| 🥇 **M14 Vitrine Matinal & Escada de Confiança** 🔄 EM ANDAMENTO | ✅ PR-A Escada backend · ✅ PR-B Escada UI · ✅ PR-C Vitrine Matinal · PR-D consolidação (polish UI + SECURITY.md + roadmap.html + audit + smoke) | Fecha o "lado humano do loop" da V2 — depois de M11 (memória) + M12 (definição) + M13 (verificação), a próxima alavanca é a confiança humana crescendo no tempo. Spec em `docs/superpowers/specs/2026-05-18-m14-vitrine-confianca-design.md`. |

**Recomendação:** seguir o M14 — PR-D (consolidação) fecha o milestone.
...
| Concluído recentemente | **M14 PR-C — Vitrine Matinal** mergeado 2026-05-20. 9 commits inline em `main`: migration **0034** (`companies.briefing_reviewed_at` + `briefing_headline_json` — additive ALTERs); shared types `Briefing` + `BriefingItem` + `Company.briefingReviewedAt`; companies repo ganha `setBriefingReviewedAt` + `setBriefingHeadline` + `getBriefingHeadlineRaw`; novo módulo `apps/main/src/briefing/`: `build.ts` (read-model SQL JOINs sobre `inbox_items`, `goals`, `goal_criteria`, `issues`, `cost_events`, janela 24h default) + `headline.ts` (geração via `claude -p` curto reusando `derivation/runner.ts` + `buildAuthEnv`, cacheada por hash SHA-256 dos 6 contadores, custo gravado `adapter_name='briefing-headline'`, fallback determinístico quando o CLI não está disponível, cache não escrito em falha); IPC `briefing:get` + `briefing:mark-reviewed`; renderer-side `useBriefingStore` zustand + nova rota `/briefing` (manchete + "Precisa de você" no topo + faixa colapsável com `verified/failed/inProgress/learned` + rodapé com custo); **landing route flipped from `/dashboard` to `/briefing`** (wildcard redirect + `/setup` success + sidebar `NavLink` prependado); 14 keys i18n (`nav.briefing` + `briefing.*`). **+16 testes** (migration 0034: 2 · build: 6 · headline: 5 · handlers: 3). Zero impacto no system prompt do agente (grep do orchestrator confirmou). Antes: **M14 PR-A + PR-B** mergeados 2026-05-20 (Escada backend + UI). |
| Testes | **1641 passing + 2 todo**, 0 lint/typecheck errors |
| Commits no main | ~845 |
...
| Restante pra v1 | **Nada — v1 fechado em 2026-05-15.** M11 + M12 + M13 fechados. **M14 em andamento** (PR-A ✅ · PR-B ✅ · PR-C ✅ · PR-D restante). |
