...
- **M12 em andamento** — PR-A mergeado (2026-05-18): `role_templates` vira biblioteca editável, rota `/roles`, charter de 8 seções por papel. PR-B mergeado (2026-05-18): Manual Operacional como skill embutida (constante compilada, injetada como entrada L0 + servida sob demanda via `skill_read`) + pointer no `preamble.md`. Planos em `docs/superpowers/plans/2026-05-18-m12-pr-*.md`.
- **1239 testes passing + 2 todo** · 0 lint/typecheck errors
- HEAD `main`: M12 PR-B (operating manual) mergeado (2026-05-18)

### ▸ Próximo

| Candidato | Escopo | Por quê |
|---|---|---|
| 🥇 **M12 Agent & Org Definition Layer** (em andamento) | ✅ PR-A charter de role (8 seções) + `/roles` biblioteca editável · ✅ PR-B Manual Operacional como skill embutida · ⬜ PR-C aba Instructions + `composeSystemPrompt` lê o charter · ⬜ PR-D CEO arquiteto + AGENTS.md charters · ⬜ PR-E `agent_runs` + budget per-agent + Run Policy · ⬜ PR-F consolidação | V2 logo após o M11 — agente bem-instruído fortalece Workflow Plays e Enforced Outcomes. Doc de design em `docs/m12-agent-org-definition-layer.md`. |

**Recomendação:** seguir o M12 — PR-C (aba Instructions + `composeSystemPrompt` lê o charter do disco).
...
| Em andamento | **M12 Agent & Org Definition Layer** — PR-A ✅ + PR-B ✅ mergeados 2026-05-18 (biblioteca de papéis + charter de 8 seções · Manual Operacional como skill embutida). Restam PR-C a PR-F. |
| Testes | **1239 passing + 2 todo**, 0 lint/typecheck errors |
| Commits no main | ~727 |
