...
>
> **Distribuição (decisão 2026-05-11):** **hybrid** — Electron desktop continua como default e UI. Adapter pattern (M7.5 foundation, M9 API key, **M10 VPS Docker**) permite spawnar agentes localmente OU em containers Docker numa VPS remota. Usuário escolhe per-agent (CEO local pra latência, engenheiros remotos pra isolamento). Sem rewrite — adapter pattern absorve o segundo lifecycle. **2026-05-26 — VPS deployment do app:** além do M10 (agentes em containers remotos), o app inteiro agora pode rodar numa VPS Linux via Docker + noVNC + Authelia 2FA — `pnpm dist:linux` gera o AppImage, `infra/docker/vps/` empacota e expõe. Path paralelo ao desktop Windows; nenhum dos dois regride.
>
...
- **Manual operacional embutido** — todo funcionário já chega sabendo "como esta empresa opera": o ciclo de vida das tarefas, quando registrar o resultado, como delegar pra outro funcionário e disciplina de custo. Esse manual vem junto com o app — você não precisa escrever nada (2026-05-18)
- **Editar as instruções de cada funcionário** (aba Instruções no Agent Studio) — cada funcionário tem um conjunto de arquivos de instrução: o charter (copiado do cargo, depois editável só pra ele) mais quaisquer arquivos extras que você adicionar. Você edita, adiciona e apaga esses arquivos num editor; eles passam a guiar de fato o comportamento daquele funcionário (2026-05-18)
- **Gerar o charter de um cargo com IA** — em vez de escrever as 8 seções na mão, você descreve o cargo numa frase ("agente que pesquisa concorrentes e entrega ao revisor") e a IA monta o charter completo pra você revisar e ajustar (2026-05-18)
- **CEO projeta a empresa inteira** — você diz ao CEO no chat "monte uma agência de tráfego" e ele propõe a organização completa: os cargos (cada um com charter), os funcionários e quem reporta a quem. A proposta cai na sua caixa de entrada; você abre a tela de revisão, marca/desmarca cargos e funcionários e aprova — só então a empresa é criada. Nunca apaga nada, só adiciona (2026-05-18)
...
- Histórico de **runs** (sessões) com timestamps + duração (2026-05-18)
- **Budget por funcionário** — defina um teto de tokens por dia ou por mês para cada agente (e um teto em USD para agentes que rodam via API key). Ao chegar a 80% do limite você recebe um aviso na inbox; a 100% o agente pausa sozinho. A aba Stats mostra o consumo atual vs. o limite (2026-05-18)
- **Run Policy por funcionário** — dois interruptores por agente: se ele pode contratar/demitir outros agentes e se pode atribuir issues a terceiros. Desligar um remove as ferramentas correspondentes no próximo spawn — útil pra restringir um agente sem mexer nas Skills globais (2026-05-18)
...
- **Deploy opcional numa VPS Linux** — `pnpm dist:linux` gera um AppImage; `infra/docker/vps/` empacota tudo num container (Xvfb + noVNC) atrás de Traefik + Authelia (2FA). Você acessa o desktop completo de qualquer navegador, do celular ao trabalho. Setup documentado passo a passo em `infra/docker/vps/README.md`.
...
| 💡 **Perguntas interativas do agente (`AskUserQuestion`)** | Renderizar a tool builtin `AskUserQuestion` do Claude como cartão de múltipla escolha (botões) na conversa, e devolver a escolha como `tool_result` pro agente continuar. Hoje ela é desabilitada via `--disallowedTools` (o agente pergunta em texto) porque a UI não a renderiza — ver `build-args.ts`. | Descoberto no M18: o CEO naturalmente quer fazer perguntas estruturadas antes de montar o org-plan; botões clicáveis seriam UX muito superior ao texto livre e combinam com a tese conversacional do V2. Precisa de: parse do tool_use, UI de opções, e um caminho novo no adapter pra enviar `tool_result` via stdin (hoje `sendInput` só manda texto de usuário). |
| 💡 **Fechar no X → minimizar pra bandeja** | Hoje clicar no X **encerra o app por completo** — o que mata os agentes rodando em background. Com a bandeja (tray) do M17 já existindo, o X deveria minimizar pra bandeja (agentes seguem trabalhando) e o "Sair" de verdade fica no menu da bandeja. Idealmente configurável em Ajustes. | Descoberto no M18: um "1-person business" com agentes assíncronos não pode parar tudo só porque a janela foi fechada. Precisa interceptar `close` na main-window (`event.preventDefault()` + `hide()`), ajustar o quit real via tray, e um flag `isQuitting`. |
| 💡 **Redesign do inbox de aprovação** | Repensar o design do inbox de aprovações como um todo (não só o `org_proposed`). Hoje é uma lista em `/inbox`; o Início não o reflete bem e propostas (org-plan/goal-plan/promoções de confiança) ficam pouco visíveis. Pensar: o inbox como centro de decisões do dono, integrado ao Início, com tipos de item bem desenhados (aprovar org, aprovar plano, aprovar gasto, promover confiança). | Descoberto no M18: o `org_proposed` ficou invisível até virar banner; sinal de que o inbox precisa de um design de primeira classe, não item-a-item improvisado. Reconciliar com a reestruturação do M16. |
| 💡 **Plano inline na conversa (split-view do M16)** | Hoje uma proposta de organização aparece como **banner → `/org-plan`** (tela de revisão completa do M12). O mockup do "Pedir algo" do M16 (`flow-pedir-plano.html`) previa um **painel do plano ao lado da conversa** (O que vai ser feito / Quem / Tempo e custo + Aprovar/Ajustar) sem trocar de tela. Polish: renderizar o resumo do org-plan (e do goal-plan) num painel lateral no PedirAlgo / chat do CEO, com Aprovar inline, reusando os dados de `OrgPlanReview`/`GoalPlanReview`. | Descoberto no M18: o banner resolve a descoberta, mas trocar de tela pra aprovar quebra o fluxo conversacional que o M16 desenhou. |
| 💡 **Login / conta de usuário** | Hoje o app não tem login próprio — abre direto e usa o token OAuth do Claude pros agentes. Adicionar um login do app (proteger o acesso à máquina/dados; possivelmente conta/sincronização no futuro). Decidir o escopo: PIN/senha local simples vs. conta na nuvem. | Pedido no M18 (uso real). Mínimo viável: gate local (senha/PIN) sobre o `userData`; o desenho de conta/multi-dispositivo fica para um spec próprio. |
| 💡 **Instalador: escolher pasta de instalação** | O NSIS hoje é one-click (`oneClick: true`) — instala em local fixo por-usuário, sem perguntar. Permitir o usuário escolher onde instalar: `oneClick: false` + `allowToChangeInstallationDirectory: true` no `electron-builder.yml` (avaliar `perMachine`). | Pedido no M18. Trade-off: one-click é mais simples/silencioso; o instalador assistido dá controle de pasta mas adiciona telas. Testar que o auto-update continua funcionando com a pasta customizada. |
...
| Em construção (local, não pushado) | **2026-05-21:** gaps M16 #2/#3/#4 (Kanban de Projetos embutido, "Pedir algo" vira conversa→plano, emoji→SVG) + fixes (scroll do composer, token OAuth headless auto-refresh) + **M17 distribuição PR-A→D** (installer electron-builder, workflow CI de release em tag, electron-updater backend, UI de update). PR-E (cortar v0.1.0 ao vivo) pendente de smoke + push. ~24 commits locais. |
| Testes | **1757 passing + 2 todo**, 0 lint/typecheck errors |
| Commits no main | ~845 |
