...
- **M12 em andamento** — PR-A (biblioteca de papéis + charter de 8 seções), PR-B (Manual Operacional como skill embutida) e PR-C (bundle de instruções por agente em disco + aba Instructions + charter injetado no system prompt) mergeados 2026-05-18. Planos em `docs/superpowers/plans/2026-05-18-m12-pr-*.md`.
- **1252 testes passing + 2 todo** · 0 lint/typecheck errors
- HEAD `main`: M12 PR-C (instruction bundle + Instructions tab) mergeado (2026-05-18)

### ▸ Próximo

| Candidato | Escopo | Por quê |
|---|---|---|
| 🥇 **M12 Agent & Org Definition Layer** (em andamento) | ✅ PR-A charter de role (8 seções) + `/roles` biblioteca editável · ✅ PR-B Manual Operacional como skill embutida · ✅ PR-C bundle de instruções por agente + aba Instructions + charter no system prompt · ⬜ PR-D CEO arquiteto + AGENTS.md charters · ⬜ PR-E `agent_runs` + budget per-agent + Run Policy · ⬜ PR-F consolidação | V2 logo após o M11 — agente bem-instruído fortalece Workflow Plays e Enforced Outcomes. Doc de design em `docs/m12-agent-org-definition-layer.md`. |

**Recomendação:** seguir o M12 — PR-D (CEO arquiteto de organização + extensão do `AGENTS.md`).
...
| Em andamento | **M12 Agent & Org Definition Layer** — PR-A ✅ + PR-B ✅ + PR-C ✅ mergeados 2026-05-18 (biblioteca de papéis + charter · Manual Operacional embutido · bundle de instruções por agente + aba Instructions + charter no system prompt). Restam PR-D a PR-F. |
| Testes | **1252 passing + 2 todo**, 0 lint/typecheck errors |
| Commits no main | ~736 |
