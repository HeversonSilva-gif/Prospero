...
- **M14 🔄 EM ANDAMENTO** — Vitrine Matinal & Escada de Confiança. **PR-A (Escada — backend) mergeado 2026-05-20**: cada funcionário tem um **degrau de confiança** (`novato → confiável → autônomo`) que sobe automaticamente conforme entrega objetivos verificados sem falhas; um funcionário "confiável" deixa de pedir aprovação pra ler arquivos do projeto (Write/Edit/Bash continuam supervisionados); a subida pra "autônomo" (que liga o modo `auto` da Run Policy) **passa por você** via um card de inbox; rebaixamento é imediato em qualquer falha de verificação (segurança vem antes de UX). Toda decisão fica auditada na nova tabela `trust_events`. **Sem UI ainda** (vem no PR-B); o motor já calibra a confiança nos bastidores. Faltam PR-B (UI da Escada), PR-C (Vitrine Matinal), PR-D (consolidação).
- **1625 testes passing + 2 todo** · 0 lint/typecheck errors
- HEAD `main`: M14 PR-A (Escada de Confiança — backend) mergeado (2026-05-20)

### ▸ Próximo

| Candidato | Escopo | Por quê |
|---|---|---|
| 🥇 **M14 Vitrine Matinal & Escada de Confiança** 🔄 EM ANDAMENTO | ✅ PR-A Escada backend (auto-aprovação read-only por degrau + motor reativo + IPC) · PR-B Escada UI (badge + histórico + card de aprovação) · PR-C Vitrine Matinal (briefing diário + manchete cacheada + virar rota inicial) · PR-D consolidação | Fecha o "lado humano do loop" da V2 — depois de M11 (memória) + M12 (definição) + M13 (verificação), a próxima alavanca é a confiança humana crescendo no tempo. Spec em `docs/superpowers/specs/2026-05-18-m14-vitrine-confianca-design.md`. |

**Recomendação:** seguir o M14 — PR-B (UI da Escada) é o próximo PR. Apresenta o `trust_tier` no agent detail e cria o fluxo de aprovação do card `trust_promotion_suggested` que o backend já dispara.
...
| Concluído recentemente | **M14 PR-A — Escada de Confiança (backend)** mergeado 2026-05-20 — abre o M14. 13 commits inline em `main` (14 tasks no plano + 1 fix de cleanup): migrações **0032** (`agents.trust_tier` + `trust_events` + `idx_trust_events_agent`) e **0033** (kind `trust_promotion_suggested` no `inbox_items` — recriação preservando `idx_inbox_company_unread`); shared types em `@prospero/shared/trust` (`TrustTier` · `TrackRecord` · `TierEvaluation` · `TrustEvent`); `evaluateTier(record, current)` puro com thresholds (`CONFIAVEL_MIN_OUTCOMES=5` · `AUTONOMO_MIN_OUTCOMES=15` · `AUTONOMO_MIN_PASS_RATE=0.9`); `collectTrackRecord` (read-only SQL sobre M13 goals/goal_criteria + M7.5 approvals + trust_events, janela de 30d); `isReadOnlyTool` classifier (Read/Glob/Grep + `list_*` + allowlist MCP); `recomputeAgentTrust` motor reativo (promove `novato→confiavel` automático · sugere `confiavel→autonomo` via card inbox · rebaixa imediato em falha + reverte `mode='supervised'` se vinha de autonomo); gate rule no `evaluatePermission` (non-novato + read-only ⇒ allow + audit `trust.readonly_autoapproved`); hook em `applyVerificationReport` (goal achieved/failed) e em `approvals.decide` (user approve/reject); IPC `trust:get-history` + `trust:approve-promotion`; 4 novas activity actions (`trust.promoted/demoted/promotion_suggested/readonly_autoapproved`). **+61 testes** (migrations 0032+0033: 5 · agents/repository: 2 · trust/repository: 2 · trust/evaluate: 9 · trust/track-record: 6 · trust/read-only-tools: 21 · trust/engine: 6 · security.gate-trust: 6 · trust-handlers: 4). Zero impacto no system prompt do agente (PR-A é puramente gate + engine + IPC). Antes: **M13 PR-A..PR-F** mergeados 2026-05-19/05-20 (M13 fechado completo). |
| Testes | **1625 passing + 2 todo**, 0 lint/typecheck errors |
| Commits no main | ~845 |
...
| Restante pra v1 | **Nada — v1 fechado em 2026-05-15.** M11 + M12 + M13 fechados. **M14 em andamento** (PR-A ✅ · PR-B/C/D restantes). |
