...
- **1738 testes passing + 2 todo** · 0 lint/typecheck errors
...
|---|---|---|
| 🥇 **M15 PR-C Consolidação** | Polish (`frontend-design` inline) · painel "Histórico de disparos" por routine (lê `activity_events` filtrado por `entity_id`) · `SECURITY.md` seção Routines · `roadmap.html` · limpeza de dead i18n keys (`routines.empty.cta`/`empty.title`) · não-regressão completa. | Fecha o M15 inteiro. Pré-req de `Workflow Plays` e `Async governance`. |

**Recomendação:** seguir pra M15 PR-C — engine + UI já rodam; falta polish e a seção de history que ficou fora do PR-B.
...
| Concluído recentemente | **M15 PR-B — UI das Routines + bug-fix `nextFireAt`** mergeado 2026-05-20. 10 tasks no plano · 13 commits em `main` (3 commits extras de fix após per-task review: pt-BR phrasing, nested-button HTML invalid, emoji rule). Entrega: (1) bug-fix do PR-A — `routines:update` re-semeia `nextFireAt` via `computeNextFire` quando `scheduleSpec` muda (mais `ROUTINE_UPDATE_INPUT_SCHEMA` ganha `nextFireAt` opcional); (2) ~46 chaves i18n `routines.*` + `nav.routines` em pt-BR e en-US, com paridade automática; (3) `lib/routines/format-summary.ts` — formatadores puros `formatScheduleSummary`/`formatEventSummary`/`parseAtMinute`/`formatAtMinute` com exhaustiveness guard, fake `t` injetável para teste; (4) `stores/routines.ts` zustand store com 7 ações (load/getById/create/update/delete/runNow + `toggleEnabled` optimistic com snapshot-revert em erro) + `env.d.ts` declarando `window.prospero.routines.*`; (5) componentes `components/routines/` — `RecurrencePicker` (4 tabs via TabBar segmented + inputs HTML5 nativos), `EventPicker` (dropdown), `TargetAgentPicker` (filtra terminated, mostra empty state inline), `RoutineRow` (linha rica `div role="button"` + toggle inline + SVG play + sub-line via formatters); (6) `routes/Routines.tsx` (lista + header com `+ Nova routine` Link + EmptyState com ClockIcon SVG quando zero) e `routes/RoutineForm.tsx` (single route que cobre `/routines/new` e `/routines/:id` via `useParams`; pre-fill em edit; client-side validation com gate do submit; banner dismissable em erro IPC; Excluir + Rodar agora só em edit; `window.confirm` no delete); (7) wire em `apps/renderer/src/App.tsx` — imports + NavLink entre Briefing e Dashboard + 3 routes. **+26 testes** (17 format-summary · 9 store + 1 bug-fix main). Zero emoji na UI (regra do usuário — SVG icons substituem `▶`/`✕`/`⏱`); paridade en/pt automática. Antes: **M15 PR-A** (engine backend) mergeado 2026-05-20. |
| Testes | **1738 passing + 2 todo**, 0 lint/typecheck errors |
| Commits no main | ~845 |
...
| Restante pra v1 | **Nada — v1 fechado em 2026-05-15.** M11 + M12 + M13 + M14 fechados. **M15 PR-A (engine) + PR-B (UI) mergeados**; resta PR-C (consolidação). |
