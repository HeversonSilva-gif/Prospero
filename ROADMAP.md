...
- **M13 🔄 EM ANDAMENTO** — Espinha de Outcome & Verificação. **PR-A (ISA) + PR-B1 (motor de verificação) mergeados 2026-05-19**: cada objetivo tem uma definição estruturada do que significa estar "pronto"; e agora, quando todas as tarefas de um objetivo terminam, os critérios são checados automaticamente — o objetivo só vira "concluído" se tudo passar, senão volta para "em andamento" com um aviso na caixa de entrada. Faltam PR-B2 (ferramentas de verificação para os agentes), PR-C TELOS, PR-D Algorithm, PR-E zonas de contenção, PR-F consolidação.
- **1437 testes passing + 2 todo** · 0 lint/typecheck errors
- HEAD `main`: M13 PR-B1 (motor de verificação) mergeado (2026-05-19)

### ▸ Próximo

| Candidato | Escopo | Por quê |
|---|---|---|
| 🥇 **M13 Espinha de Outcome & Verificação** 🔄 EM ANDAMENTO | ✅ PR-A ISA (definição estruturada de "pronto") · ✅ PR-B1 motor de verificação (checagem automática + gate de conclusão) · PR-B2 ferramentas de verificação para os agentes · PR-C TELOS (norte da empresa) · PR-D The Algorithm · PR-E zonas de contenção · PR-F consolidação | Concretiza a aposta "Enforced Outcomes" da V2 — torna o "feito" um fato verificável, não uma opinião. Doc de design em `docs/superpowers/specs/2026-05-18-m13-outcome-verification-spine-design.md`. |

**Recomendação:** seguir o M13 — PR-B2 (ferramentas de verificação para os agentes) é o próximo PR.
...
| Concluído recentemente | **M13 PR-B1 — motor de verificação** mergeado 2026-05-19. Quando todas as tarefas de um objetivo terminam, ele entra em "verificando", os critérios determinísticos são checados automaticamente (comando em sandbox, artefato, métrica) e o objetivo só vira "concluído" se tudo passar — senão volta para "em andamento" com um aviso. Critérios de julgamento são resolvidos pela pessoa. Antes: **M13 PR-A — ISA** mergeado 2026-05-19. |
| Testes | **1437 passing + 2 todo**, 0 lint/typecheck errors |
| Commits no main | ~810 |
...
| Restante pra v1 | **Nada — v1 fechado em 2026-05-15.** M11 + M12 fechados. **M13 em andamento** (PR-A ✅ · PR-B1 ✅, faltam PR-B2, C, D, E, F). |
