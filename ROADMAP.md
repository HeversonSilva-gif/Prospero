...
- **M13 🔄 EM ANDAMENTO** — Espinha de Outcome & Verificação. **PR-A (ISA) + PR-B (motor de verificação, B1+B2) + PR-C (TELOS) + PR-D (The Algorithm) mergeados 2026-05-19**: cada objetivo tem uma definição estruturada do que significa estar "pronto"; quando todas as tarefas terminam, os critérios são checados automaticamente — o objetivo só vira "concluído" se tudo passar, senão volta para "em andamento" com um aviso; os agentes podem checar/julgar critérios eles mesmos; cada empresa tem um **TELOS** — o norte do negócio — que passa a guiar todo agente; e agora todo agente carrega no contexto **The Algorithm** — o loop operacional de 7 fases (observar → pensar → planejar → construir → executar → verificar → aprender) entregue como skill embutida, com VERIFY ligado ao motor de verificação e LEARN alimentando o pipeline de aprendizado da empresa com sinal mais rico (quantas tentativas cada critério levou; falhas de verificação viram gatilho de derivação). Faltam PR-E zonas de contenção e PR-F consolidação.
- **1517 testes passing + 2 todo** · 0 lint/typecheck errors
- HEAD `main`: M13 PR-D (The Algorithm — o loop operacional do agente) mergeado (2026-05-19)

### ▸ Próximo

| Candidato | Escopo | Por quê |
|---|---|---|
| 🥇 **M13 Espinha de Outcome & Verificação** 🔄 EM ANDAMENTO | ✅ PR-A ISA (definição estruturada de "pronto") · ✅ PR-B motor de verificação (checagem automática + gate de conclusão + ferramentas de verificação para os agentes) · ✅ PR-C TELOS (norte da empresa) · ✅ PR-D The Algorithm (loop operacional de 7 fases) · PR-E zonas de contenção · PR-F consolidação | Concretiza a aposta "Enforced Outcomes" da V2 — torna o "feito" um fato verificável, não uma opinião. Doc de design em `docs/superpowers/specs/2026-05-18-m13-outcome-verification-spine-design.md`. |

**Recomendação:** seguir o M13 — PR-E (zonas de contenção — privacidade estrutural cross-empresa) é o próximo PR.
...
| Concluído recentemente | **M13 PR-D — The Algorithm** mergeado 2026-05-19. Todo agente carrega no contexto o loop operacional de 7 fases (observar → pensar → planejar → construir → executar → verificar → aprender) como skill embutida (`algorithm`) — entregue exatamente como o Manual Operacional (M12 PR-B): constante compilada + entrada L0 sintética + `skill_read` por fallback (~110 chars no system prompt; corpo on-demand). VERIFY é a fase dura — instrui o agente a chamar `criterion_check` antes de marcar uma tarefa pronta, mas o gate real é o motor de verificação. LEARN alimenta o pipeline de aprendizado da empresa com sinal mais rico: cada critério agora tem contador de tentativas (`goal_criteria.attempts`), e o prompt de retrospectiva e de issue-done renderiza "passou de primeira" / "passou após N tentativas" / "ainda falhando"; falha de verificação vira o quinto gatilho de derivação (`verification.failed` → skill candidata "como evitar essa falha"). Antes: **M13 PR-C — TELOS**, **PR-B2/B1 — motor de verificação**, **PR-A — ISA**, todos 2026-05-19. |
| Testes | **1517 passing + 2 todo**, 0 lint/typecheck errors |
| Commits no main | ~845 |
...
| Restante pra v1 | **Nada — v1 fechado em 2026-05-15.** M11 + M12 fechados. **M13 em andamento** (PR-A ✅ · PR-B ✅ · PR-C ✅ · PR-D ✅, faltam PR-E, F). |
