...
Run: `pnpm --filter @dashboard-agent/main test db.migration-0005`
...
Run: `pnpm --filter @dashboard-agent/main test projects.repository.slug`
...
Run: `pnpm --filter @dashboard-agent/main test issues.repository.identifier`
...
Run: `pnpm --filter @dashboard-agent/main test db.post-migration-0005`
...
Run: `pnpm --filter @dashboard-agent/main test mcp.tools.list-issues-identifier`
...
Run: `pnpm --filter @dashboard-agent/main build`
...
Run: `pnpm --filter @dashboard-agent/main test db.migration-0006`
...
Run: `pnpm --filter @dashboard-agent/main test messages.repository.kind`
Expected: FAIL.
...
(b) Extend `rowToMessage` to include `kind: r.kind as MessageKind` (import `MessageKind` from `@dashboard-agent/shared`).
...
Run: `pnpm --filter @dashboard-agent/main test messages.unanswered-questions`
...
import type { MessageKind } from "@dashboard-agent/shared";
```
...
Run: `pnpm --filter @dashboard-agent/main test db.migration-0007`
Expected: FAIL.
...
Note: we keep `Approval` types local to `apps/main` for now to avoid a workspace re-export ceremony — the renderer does not yet consume them. They'll be promoted to `@dashboard-agent/shared` in PR-C when the inbox surfaces them in UI.
...
Run: `pnpm --filter @dashboard-agent/main test approvals.repository`
...
Run: `pnpm --filter @dashboard-agent/main test mcp.tools.request-permission-approvals`
...
Run: `pnpm --filter @dashboard-agent/main test inbox.dual-format`
...
Run: `pnpm --filter @dashboard-agent/main test db.migration-0008`
Expected: FAIL.
...
Run: `pnpm --filter @dashboard-agent/main test artifacts.repository`
Expected: FAIL.
...
Run: `pnpm --filter @dashboard-agent/main test mcp.tools.record-artifact`
...
Run: `pnpm --filter @dashboard-agent/main build`. Confirm `dist/orchestrator/preamble.md` is updated.
...
Run: `pnpm --filter @dashboard-agent/main test --run`
Expected: 310+ → ~340+ tests passing. No previously-green tests should turn red.

- [ ] **Step 2: Run renderer test suite (if it has one)**

Run: `pnpm --filter @dashboard-agent/renderer test --run 2>NUL` (skip if filter is absent)
Expected: PASS.
...
Run the M6.1 smoke procedure documented under [project_m6_lessons](../../../../C:/Users/hever/.claude/projects/D--Projetos-pessoais-DashboardAgent/memory/project_m6_lessons.md):
- Launch the app
- Hire an engineer
- Send it a message
...
Run: `pnpm --filter @dashboard-agent/main test token-leak sandbox-escape gate`
Expected: PASS.
