...
- Free-text body (after the `---` close) is ignored on import and emitted as a static "Generated by DashboardAgent" header on export.
...
Run:
```bash
pnpm --filter @dashboard-agent/main add gray-matter@^4.0.3
```
...
Run:
```bash
pnpm --filter @dashboard-agent/main test agents-md/parser.test
```
...
Run:
```bash
pnpm --filter @dashboard-agent/main test agents-md/serialize.test
```
...
const BODY = `# DashboardAgent — Team manifest

This file was generated by DashboardAgent. Edit the front-matter above to add
...
Run:
```bash
pnpm --filter @dashboard-agent/main test agents-md/hire.test
```
...
Run:
```bash
pnpm --filter @dashboard-agent/main test agents-md/build-export.test
```
...
Run:
```bash
pnpm --filter @dashboard-agent/main test ipc/agents-md-handlers.test
```
...
Run:
```bash
pnpm --filter @dashboard-agent/renderer test i18n/parity.test
```
Expected: PASS.
