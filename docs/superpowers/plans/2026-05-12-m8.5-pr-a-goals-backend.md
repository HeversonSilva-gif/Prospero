...
**Test commands:**
- Main unit/integration: `pnpm --filter @dashboard-agent/main test -- --run`
- Shared: `pnpm --filter @dashboard-agent/shared test -- --run`
- Single test: `pnpm --filter @dashboard-agent/main test -- --run <pattern>`
- Lint: `pnpm lint`
...
```bash
pnpm typecheck
pnpm --filter @dashboard-agent/main test -- --run apps/main/tests/db.migrations
```
...
```bash
pnpm typecheck
pnpm --filter @dashboard-agent/shared test -- --run
```
...
```bash
pnpm --filter @dashboard-agent/main test -- --run activity
```
...
```bash
pnpm --filter @dashboard-agent/shared test -- --run goalPlan-schema
```
...
```bash
pnpm --filter @dashboard-agent/main test -- --run goals/repository
```
...
```bash
pnpm --filter @dashboard-agent/main test -- --run goals/plans-repository
```
...
```bash
pnpm --filter @dashboard-agent/main test -- --run goals/format-request
```
...
```bash
pnpm --filter @dashboard-agent/main test -- --run costs/baseline
```
...
```bash
pnpm --filter @dashboard-agent/main test -- --run tools-goals
```
...
```ts
import { GoalPlanPayloadSchema } from "@dashboard-agent/shared";
...
```ts
import { GoalPlanPayloadSchema, type GoalPlanPayload } from "@dashboard-agent/shared";
...
```bash
pnpm typecheck
pnpm --filter @dashboard-agent/main test -- --run
```
...
```bash
pnpm --filter @dashboard-agent/main test -- --run goals/executor
```
...
```bash
pnpm --filter @dashboard-agent/main test -- --run goals/recovery
```
...
```bash
pnpm --filter @dashboard-agent/main test -- --run ipc.goals-handlers
```
...
import type { Goal, GoalWithPlan, CreateGoalInput } from "@dashboard-agent/shared";
import { createGoalsRepository } from "../goals/repository.js";
import { createGoalPlansRepository } from "../goals/plans-repository.js";
import { createAgentsRepository } from "../agents/repository.js";
...
```bash
pnpm --filter @dashboard-agent/main test -- --run integration/goal-flow
```
...
pnpm --filter @dashboard-agent/main test -- --run
pnpm --filter @dashboard-agent/shared test -- --run
pnpm --filter @dashboard-agent/renderer test -- --run
```
